...
void drawline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,uint8_t color) {
	if(x0 == x1)
	{
		unsigned int i;
		for(i=y0;i<=y1;i++)
			setpixel(buff,x0,i,1);
	}
	else if(y0 == y1)
	{
		unsigned int i;
		for(i=x0;i<=x1;i++)
			setpixel(buff,i,y0,1);
	}

}

void clearline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,uint8_t color) {
	if(x0 == x1)
	{
		unsigned int i;
		for(i=y0;i<=y1;i++)
		clearpixel(buff,x0,i);
	}
	else if(y0 == y1)
	{
		unsigned int i;
...
void fillrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	unsigned int i;
	for(i=y;i<(y+h);i++)
		drawline(buff,x,i,x+w-1,i,1);
}

// function to draw a rectangle
void drawrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	drawline(buff,x,y,x,y+h-1,1);
	drawline(buff,x+w-1,y,x+w-1,y+h-1,1);
	drawline(buff,x,y,x+w-1,y,1);
	drawline(buff,x,y+h-1,x+w-1,y+h-1,1);
}


void Circle(int x0, int y0, int x, int y)
{
	setpixel(buff,x0 + x, y0 + y,1);
	setpixel(buff,x0 + y, y0 + x,1);
	setpixel(buff,x0 - y, y0 + x,1);
	setpixel(buff,x0 - x, y0 + y,1);
	setpixel(buff,x0 - x, y0 - y,1);
	setpixel(buff,x0 - y, y0 - x,1);
...
void Fcircle(int xc, int yc, int x, int y){
	...
	drawline(buff,xc+x, yc-y, xc+x, yc+y,1);
	//setpixel(buff,xc+x, yc-y, 1);
	
	drawline(buff,xc-x, yc-y, xc-x, yc+y,1);
	//setpixel(buff,xc-x, yc-y, 1);

	drawline(buff,xc+y, yc-x, xc+y, yc+x, 1);
	//setpixel(buff,xc+y, yc-x, 1);
